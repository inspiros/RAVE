...
def get_mask(indices, r, tmin=None, tmax=None):
    if tmin is None:
        tmin = 50_000
    if tmax is None:
        tmax = indices.shape[0]
    tmax = int(tmax)
    n = int(r * (tmax - tmin) + tmin)
    mask = torch.zeros(indices.shape, dtype=torch.bool, device=indices.device)
    mask[indices[:n]] = True
    return mask


def training(scene, dataset, opt, pipe, saving_iterations, checkpoint_iterations, debug_from, wandb=None, logger=None):
    first_iter = 30_000
    ...
    # precompute prune indices
    indices = get_sort_indices(scene, dataset, opt, pipe)

    if opt.freeze_anchors:
        gaussians.freeze_anchors(remove_params=True)
    if opt.freeze_mlp:
        gaussians.freeze_mlp(remove_params=True)

    for iteration in range(first_iter, first_iter + opt.iterations + 1):

        # update masks
        if opt.update_masks_freq and (iteration != first_iter and
                                     (iteration - first_iter) % opt.update_masks_freq == 0):
            logger.info("\n[ITER {}] Updating masks".format(iteration))
            indices = get_sort_indices(scene, dataset, opt, pipe)

        r = torch.rand([]).item()
        if args.sanity_check:  # just for checking
            progr = np.flip(geometric_progression(1, args.num_levels, args.num_levels))
            probabilities = torch.tensor(progr / progr.sum(), dtype=torch.float32)
            r = torch.linspace(0, 1, args.num_levels)[
                torch.multinomial(probabilities, 1).item()].item()
            print('sampled rate:', r)
        mask = get_mask(indices, r, args.min, indices.shape[0] * args.max)
        ...
        with torch.no_grad():
            ...
            #         test(None, dataset_name, iteration, Ll1, loss, l1_loss, iter_start.elapsed_time(iter_end),
...
def get_sort_indices(scene, dataset, opt, pipe):
    gaussians = scene.gaussians
    bg_color = [1, 1, 1] if dataset.white_background else [0, 0, 0]
    background = torch.tensor(bg_color, dtype=torch.float32, device="cuda")
    ...
    indices = scene.gaussians.prune_sort_indices(prune_type=args.prune_type)
    gaussians.optimizer.zero_grad(set_to_none=True)
    return indices


@torch.no_grad()
def test(tb_writer, dataset_name, iteration, scene : Scene, renderFunc, renderArgs, wandb=None, logger=None, anchor_mask=None, level=None, eval_lpips=False, dump_images=False, size=None):
...
@torch.no_grad()
def save_model(scene, dataset, pipe, dataset_name, iteration, indices, num_levels=100):
    gaussians = scene.gaussians
    ...
    save_paths = []
    for i, t in enumerate(torch.linspace(0, 1, num_levels, dtype=torch.float32)):
        mask = get_mask(indices, t, args.min, indices.shape[0] * args.max)
    ...
    torch.save(indices, os.path.join(args.model_path, 'anchor_indices.pt'))
    ...
    for i, path in enumerate(save_paths):
        ...
        test(None, dataset_name, iteration, scene, render, (pipe, background), wandb, logger, None,
             eval_lpips=True, dump_images=True, level=level, size=size)
...
if __name__ == "__main__":
    ...
    if args.test_only:
        ...
        anchor_indices = training(scene, lp.extract(args), op.extract(args), pp.extract(args), args.save_iterations, args.checkpoint_iterations,
                                  args.debug_from, wandb, logger)
        save_model(scene, lp.extract(args), pp.extract(args), dataset, 'final', anchor_indices, args.num_levels)
